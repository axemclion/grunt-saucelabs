function getPreviousTests( rTestName, rModuleName ) {
  let testSpan; let moduleSpan;
  const matches = [];
  let i = 0;
  const rModule = /(^| )module-name( |$)/;
  const testNames = typeof document.getElementsByClassName !== 'undefined' ?
			document.getElementsByClassName('test-name') :
			(function( spans ) {
			  let span;
			  const tests = [];
			  let i = 0;
			  const rTest = /(^| )test-name( |$)/;
			  for ( ; (span = spans[i]); i++ ) {
...
function makeurl() {
  const testEnv = QUnit.current_testEnvironment;
  const url = testEnv.url || 'http://example.com/search';
  const q = testEnv.q || 'a search test';
...
test('propEqual', 5, function( assert ) {
  const objectCreate = Object.create || function( origin ) {
    function O() {}
    O.prototype = origin;
    const r = new O();
    return r;
...
if (typeof document !== 'undefined') {
  ...
  test('running test name displayed', function() {
    ...
    const displaying = document.getElementById('qunit-testresult');
  ...
  (function() {
    ...
    test('basics', 2, function() {
      const previous = getPreviousTests(/^setup$/, /^timing$/)[0];
      const runtime = previous.lastChild.previousSibling;
    ...
    test('values', 2, function() {
      const basics = getPreviousTests(/^setup$/, /^timing$/)[0];
      const slow = getPreviousTests(/^basics$/, /^timing$/)[0];
...
test('check jsDump recursion', function() {
  ...
  const noref = chainwrap(0);
  const nodump = QUnit.jsDump.parse(noref);
  equal(nodump, '{\n  "first": true,\n  "wrap": undefined\n}');

  const selfref = chainwrap(1);
  const selfdump = QUnit.jsDump.parse(selfref);
  equal(selfdump, '{\n  "first": true,\n  "wrap": recursion(-1)\n}');

  const parentref = chainwrap(2);
  const parentdump = QUnit.jsDump.parse(parentref);
  equal(parentdump, '{\n  "wrap": {\n    "first": true,\n    "wrap": recursion(-2)\n  }\n}');

  const circref = chainwrap(10);
  const circdump = QUnit.jsDump.parse(circref);
  ok(new RegExp('recursion\\(-10\\)').test(circdump), '(' +circdump + ') should show -10 recursion level');
});

test('check (deep-)equal recursion', function() {
  const noRecursion = chainwrap(0);
...
test('Circular reference with arrays', function() {
  ...
  const arrdump = QUnit.jsDump.parse(arr);
  ...
  const objdump = QUnit.jsDump.parse(obj);
  const childarrdump = QUnit.jsDump.parse(childarr);
...
function testAfterDone() {
  const testName = 'ensure has correct number of assertions';
  ...
  function secondAfterDoneTest() {
    ...
    test('count previous two test\'s assertions', function() {
      const tests = getPreviousTests(/^ensure has correct number of assertions/, /^Synchronous test after load of page$/);
