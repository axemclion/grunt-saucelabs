...
The `saucelabs-jasmine` runs [Jasmine](https://jasmine.github.io/) tests in the Sauce Labs browser. The `saucelabs-jasmine` task requires `jasmine-1.3.0`. There is also a `saucelabs-mocha` task that lets you run your Mocha tests on Sauce Labs cloudified browser environment.
