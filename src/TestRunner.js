...
module.exports = function(grunt) {
   ...
   */
  const TestRunner = function(properties, framework, onProgress) {
  ...
  */
   ...
   *
   * @return {Object} - A promise which will be eventually resolved with the test results
   *   (a boolean). Progress is reported after each job is started and completed.
   */
  TestRunner.prototype.runTests = function() {
    const me = this;
    let throttledRunTest; let promises;
    ...
    return Q
        ...
        .then(function(results) {
          const passed = results.indexOf(false) === -1;
  ...
  TestRunner.prototype.runTest = function(browser, url) {
    ...
    function getResult() {
      const job = new Job(me, url, browser);
    ...
    return Q
        ...
        .then(function(result) {
          if (me.onTestComplete) {
            const clone = _.clone(result, true);
            return Q
